...
/**
 * RecyclerView ItemDecoration that draws horizontal fade gradients
 * on the left and right edges of a horizontally scrolling list.
 *
 * Left fade: semi-transparent to transparent (items hidden on left)
 * Right fade: transparent to semi-transparent (items hidden on right)
 *
 * Originally: ScrollFadeDecoration.java (39 lines)
 */
class ScrollFadeDecoration : RecyclerView.ItemDecoration() {
    ...
    override fun onDrawOver(canvas: Canvas, recyclerView: RecyclerView, state: RecyclerView.State) {
        ...
        // Left fade: items hidden on the left
        if (firstVisible > 0) {
        ...
        // Right fade: items hidden on the right
