...
/**
 * Custom view that renders a full neumorphic (soft UI) audio player card.
 *
 * The card includes:
 * - A neumorphic rounded rectangle background
 * - A central circle with chapter name (الكهف) and reciter info
 * - A progress bar with accent fill
 * - Three control buttons (rewind, pause, fast-forward)
 * - A "Created by" footer
 * - Back and settings buttons in the top bar
 *
 * The theme color can be changed via [setBaseThemeColor].
 *
 * Originally: NeumorphicView.java (193 lines)
 */
